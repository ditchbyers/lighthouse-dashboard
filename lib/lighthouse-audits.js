...
export async function loadRunWorkspace(runFolder) {
  ...
  const reportRows = withReports.filter((row) => row.lhr && row.lhr.audits);
  const validRows = reportRows.length ? reportRows : withReports;
  const firstLhr = reportRows[0]?.lhr ?? null;
  ...
  for (const [categoryId, category] of Object.entries(firstLhr?.categories ?? {})) {
   ...
   const categoryDefinitions = firstLhr?.categories
     ? Object.entries(firstLhr.categories).map(([id, category]) => ({ id, title: String(category?.title ?? id), field: null }))
     : [
         { id: 'performance', title: 'Performance', field: 'categoryPerformance' },
         { id: 'accessibility', title: 'Accessibility', field: 'categoryAccessibility' },
         { id: 'best-practices', title: 'Best Practices', field: 'categoryBestPractices' },
         { id: 'seo', title: 'SEO', field: 'categorySeo' },
       ];

   const categoryStats = categoryDefinitions.map(({ id, title, field }) => {
     const values = validRows
       .map((row) => toMetricNumber(field ? row[field] : row.lhr?.categories?.[id]?.score))
