...
import { createMockLighthouseDataset } from './lighthouse-mock';
...
async function readJsonDetails(jsonPath) {
  ...
  const resourceCount = Array.isArray(resourceItems)
    ? resourceItems.reduce((sum, item) => sum + (Number(item?.requestCount) || 0), 0)
    : 0;
...
export async function loadLighthouseDataset() {
  try {
    ...
    if (!runs.length || !rows.length) {
      return createMockLighthouseDataset();
    }
    ...
    return {
      ...
      source: 'filesystem',
      sourceLabel: 'External Lighthouse result files',
    };
  } catch {
    return createMockLighthouseDataset();
  }
